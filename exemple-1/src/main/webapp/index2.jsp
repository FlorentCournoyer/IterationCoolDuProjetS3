<%@ page import="ca.usherbrooke.gegi.server.business.Notification" %>
<head>
    <!-- Barre de titre -->
...
<script type = "text/babel">
    ...
    const banderoleElement =
        ...
        function CreeNotification(expediteur, date, description, link, id) {
            for (i = stateIndexArrayNotification.nombreNotification; i > 0; i--) {
                ...
                arrayNotification[i].id = arrayNotification[i-1].id
            }
        ...
        const interval = setInterval(function(){
            fetch('http://localhost:8080/exemple-1/api/etudiant')
                .then((response) => {
                    return response.json();
                })
                .then((myJson) => {
                    console.log(myJson);
                    const notifs = JSON.parse(JSON.stringify(myJson))
                    const stateIsNew = {isNew: 1}
                    for(let x = 0; x < notifs.length; x++){
                        Object.assign(stateIsNew, {isNew: 1})
                        for (let j = 0; j < arrayNotification.length; j++) {
                            if (notifs[x].id === arrayNotification[j].id) {
                                Object.assign(stateIsNew, {isNew: 0})
                            }
                        }
                        if (stateIsNew.isNew === 1) {
                            ReceptionNotification(notifs[x].expediteurId, notifs[x].date, notifs[x].description, notifs[x].lien, notifs[x].id);
                        }
                    }
                });
        },5000);
        ...
        function AfficherNotification() {
            for (i = 0; i < stateIndexArrayNotification.nombreNotification; i++) {
                idRow = 'td-'+i
                if (arrayNotification[i].expediteur === 1) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Horarius' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                } else if (arrayNotification[i].expediteur === 2) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Outlook' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                } else if (arrayNotification[i].expediteur === 3) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Site session' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                } else if (arrayNotification[i].expediteur === 4) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Portail' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                } else if (arrayNotification[i].expediteur === 5) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Teams' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                } else if (arrayNotification[i].expediteur === 6) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Moodle' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                } else if (arrayNotification[i].expediteur === 7) {
                    document.getElementById(idRow).innerHTML = '<button class="boutton-notification" onclick=window.open("' + arrayNotification[i].link + '")>' + 'Trivio' + '<br>' + '<br>' + arrayNotification[i].description + '<br>' + '<br>' + arrayNotification[i].date + '</button>'
                }
            }
        }
